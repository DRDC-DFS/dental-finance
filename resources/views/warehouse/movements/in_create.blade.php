...
<h4 class="mb-4">{{ isset($movement) ? 'Edit Gudang Masuk' : 'Tambah Gudang Masuk' }}</h4>
...
<form method="POST" action="{{ isset($movement) ? route('warehouse.movements.update', ['type' => 'in', 'id' => $movement->id]) : route('warehouse.movements.store', ['type' => 'in']) }}">
@csrf
@if(isset($movement))
    @method('PUT')
@endif
...
<div class="card mb-4">
    <div class="card-body">
        ...
        <div class="mb-3">
            ...
            <select name="item_id" class="form-control" required>
                ...
                @foreach($items as $id => $name)
                    <option value="{{ $id }}" {{ (string) old('item_id', $movement->item_id ?? '') === (string) $id ? 'selected' : '' }}>
                        {{ $name }}
                    </option>
                @endforeach
            </select>
        ...
        <a href="{{ route('warehouse.movements.index', ['type' => 'in']) }}" class="btn btn-secondary">
            Kembali
        </a>
...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <h5 class="mb-0">Riwayat Gudang Masuk (20 terakhir)</h5>
            <a href="{{ route('warehouse.movements.index', ['type' => 'in']) }}" class="btn btn-outline-primary btn-sm">Lihat Semua</a>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered align-middle mb-0">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center text-muted">Belum ada data gudang masuk.</td>
                    </tr>
                @endforelse
