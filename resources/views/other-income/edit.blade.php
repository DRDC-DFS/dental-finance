...
@section('title', 'Edit Pemasukan Lain-lain')

@section('content')
<div class="container-fluid py-2">

    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <h4 class="mb-0">Edit Pemasukan Lain-lain</h4>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            ...
            <div class="alert alert-info py-2">
                ...
                Data ini adalah pemasukan <strong>non-pasien</strong>, tidak mempengaruhi invoice, QR, tindakan, maupun pembayaran pasien.
            </div>

            <form method="POST" action="{{ route('other_income.update', $otherIncome->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date"
                               name="trx_date"
                               class="form-control"
                               value="{{ old('trx_date', optional($otherIncome->trx_date)->format('Y-m-d')) }}"
                               required>
                    </div>
                    ...
                    <div class="col-md-5">
                        ...
                        <input type="text"
                               name="title"
                               class="form-control"
                               value="{{ old('title', $otherIncome->title) }}"
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text"
                               name="source_type"
                               class="form-control"
                               value="{{ old('source_type', $otherIncome->source_type) }}"
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text"
                               ...
                               id="amount"
                               class="form-control"
                               value="{{ old('amount', number_format((float) $otherIncome->amount, 0, ',', '.')) }}"
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="payment_method" id="payment_method" class="form-select" required>
                            <option value="cash" @selected(old('payment_method', $otherIncome->payment_method) === 'cash')>Tunai</option>
                            <option value="bank" @selected(old('payment_method', $otherIncome->payment_method) === 'bank')>Bank</option>
                        </select>
                    </div>

                    <div class="col-md-3"
                         id="bank_name_wrapper"
                         style="{{ old('payment_method', $otherIncome->payment_method) === 'bank' ? '' : 'display:none;' }}">
                        <label class="form-label">Bank</label>
                        <select name="bank_name" id="bank_name" class="form-select">
                            <option value="">- pilih bank -</option>
                            <option value="BCA" @selected(old('bank_name', $otherIncome->bank_name) === 'BCA')>BCA</option>
                            <option value="BNI" @selected(old('bank_name', $otherIncome->bank_name) === 'BNI')>BNI</option>
                            <option value="BRI" @selected(old('bank_name', $otherIncome->bank_name) === 'BRI')>BRI</option>
                        </select>
                    </div>

                    <div class="col-md-3"
                         id="payment_channel_wrapper"
                         style="{{ old('payment_method', $otherIncome->payment_method) === 'bank' ? '' : 'display:none;' }}">
                        <label class="form-label">Channel Bank</label>
                        <select name="payment_channel" id="payment_channel" class="form-select">
                            <option value="">- pilih channel -</option>
                            <option value="transfer" @selected(old('payment_channel', $otherIncome->payment_channel) === 'transfer')>TRANSFER</option>
                            <option value="qris" @selected(old('payment_channel', $otherIncome->payment_channel) === 'qris')>QRIS</option>
                            <option value="edc" @selected(old('payment_channel', $otherIncome->payment_channel) === 'edc')>EDC</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="visibility" class="form-select" required>
                            <option value="public" @selected(old('visibility', $otherIncome->visibility) === 'public')>
                                Public
                            </option>
                            <option value="private" @selected(old('visibility', $otherIncome->visibility) === 'private')>
                                Private (Owner saja)
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="include_in_report" class="form-select" required>
                            <option value="1" @selected((string) old('include_in_report', $otherIncome->include_in_report ? '1' : '0') === '1')>Ya</option>
                            <option value="0" @selected((string) old('include_in_report', $otherIncome->include_in_report ? '1' : '0') === '0')>Tidak</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="include_in_cashflow" class="form-select" required>
                            <option value="1" @selected((string) old('include_in_cashflow', $otherIncome->include_in_cashflow ? '1' : '0') === '1')>Ya</option>
                            <option value="0" @selected((string) old('include_in_cashflow', $otherIncome->include_in_cashflow ? '1' : '0') === '0')>Tidak</option>
                        </select>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="notes"
                                  class="form-control"
                                  rows="4"
                                  placeholder="catatan tambahan untuk pemasukan ini">{{ old('notes', $otherIncome->notes) }}</textarea>
                    </div>
