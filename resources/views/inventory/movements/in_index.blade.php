...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
    <div>
        <h4 class="mb-0">Inventori Masuk</h4>
        <div class="text-muted small">{{ $periodLabel ?? '' }}</div>
    </div>

    <a href="{{ route('inventory.movements.create', ['type' => 'in']) }}" class="btn btn-primary btn-sm">
        + Tambah Masuk
    </a>
</div>
...
<div class="card mb-3">
    <div class="card-body">
        <form method="GET" action="{{ route('inventory.movements.index', ['type' => 'in']) }}" class="row g-2 align-items-end">
            @if($isOwner ?? false)
                <div class="col-md-3">
            ...
            <div class="col-md-2">
                <a href="{{ route('inventory.movements.index', ['type' => 'in']) }}" class="btn btn-outline-secondary w-100">
                    Reset
                </a>
            </div>
        ...
        <div class="small text-muted mt-2">
            @if($isOwner ?? false)
                OWNER dapat memilih rentang tanggal untuk melihat data inventori masuk.
            @else
                ADMIN hanya menggunakan 1 tanggal untuk melihat data inventori masuk.
            @endif
        </div>
    </div>
</div>
...
<div class="card">
    <div class="card-body">
        ...
        @if($movements->count() === 0)
            <div class="text-muted">Belum ada data inventori masuk pada filter yang dipilih.</div>
        @else
                ...
                <table class="table table-bordered align-middle mb-0">
                    ...
                    <tfoot class="table-light">
                        <tr>
                            ...
                            <th class="text-end">
                                {{ number_format((float) $movements->sum('qty'), 2, ',', '.') }}
                            </th>
                            <th colspan="3"></th>
                        </tr>
                    </tfoot>
